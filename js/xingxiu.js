...
(function () {
  ...
  /* ---------- 星图 ---------- */
  function buildRing() {
    var s = svg('svg', { 'class': 'xx-svg', viewBox: '-40 -40 800 800', role: 'group', 'aria-label': '二十八宿圆形星图，点击星宿或四象查看详情' });
    ...
    [['南', CX, -18], ['北', CX, 738], ['东', -18, CY], ['西', 738, CY]].forEach(function (c) {
      s.appendChild(svg('text', { x: c[1], y: c[2], 'class': 'xx-compass' }, c[0]));
    });
    return s;
  }
  ...
  function renderPanel() {
    ...
    if (state.kind === 'xiu') {
      ...
      panel.appendChild(el('div', { 'class': 'xx-panel-kicker', text: b.dir + '方' + b.name + ' · 第 ' + (b.xiu.indexOf(x.name) + 1) + ' 宿' }));
      ...
      panel.appendChild(el('div', { 'class': 'xx-stepper' }, [
      ...
      panel.appendChild(el('div', { 'class': 'xx-panel-kicker', text: q.dir + '方 · ' + q.season + ' · ' + q.color + '色' }));
      var tt = el('h2', { 'class': 'xx-panel-title', text: q.name });
      tt.style.color = BEAST_COLOR[q.name];
      panel.appendChild(tt);
      panel.appendChild(el('p', { 'class': 'xx-panel-text', text: q.focus }));
      ...
      panel.appendChild(el('div', { 'class': 'xx-panel-kicker', text: '圆圈中间的北天' }));
      panel.appendChild(el('h2', { 'class': 'xx-panel-title', text: D.center.title }));
      panel.appendChild(el('p', { 'class': 'xx-panel-text', text: D.center.text }));
    }
  }
  ...
  function render() {
    ...
    var fig = el('figure', { 'class': 'xx-figure' }, [
      buildRing(),
      el('figcaption', { 'class': 'xx-cap' }, [seasonLine, document.createTextNode(D.orient)])
    ]);
    page.appendChild(el('section', { 'class': 'xx-explore', 'aria-label': '二十八宿星图' }, [fig, panel]));
