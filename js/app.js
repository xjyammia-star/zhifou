...
(function () {
  ...
  /* ---------- 首页：抽一张（真正点击抽取） ----------
     ...
     规则：先在七个板块里等概率选一个，再从这个板块里选一张；最近抽过的十几张不会立刻重复。
     每天第一次抽到的是“今日一签”，记在浏览器里，当天再打开还是它；之后可以随手再抽。 */
  var CARDS = window.ZHIFOU_CARDS || null;
  var PLATE_KEYS = ['shi', 'shen', 'jianzhu', 'wu', 'ziyu', 'lisi', 'yi'];
  ...
  function buildDraw() {
    ...
    return el('section', { 'class': 'draw', 'aria-label': '今日一签' }, [
      el('div', { 'class': 'draw-text' }, [
        ...
        el('p', { 'class': 'draw-sub', text: '一位神、一个词、一件器物，或者一个常被说错的说法，从时、神、建筑、物、字·语、礼与思、艺七个板块里随手抽出来。' }),
        hint,
        again
      ]),
